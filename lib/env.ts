...
export const envSchema = z.object({
  ...
  SPOTIFY_DEBUG: z.string().transform(val => val === 'true' || val === '1').default('false'),
  CI: z.string().transform(val => val === 'true').default('false'),
  GOOGLE_DOC_WORKOUT_URL: z.string().url(),
  NEXT_PUBLIC_USE_NATIVE_TABLE: z.string().transform(val => val === 'true').default('false'),
  NEXT_PUBLIC_API_URL: z.string().optional(),
  NEXT_PUBLIC_WS_URL: z.string().optional(),
  TESTING: z.string().transform(val => val === 'true').default('false'),
