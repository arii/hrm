      ...
      if (!refreshedToken.error) {
        try {
          ...
          const tokenPayload: SpotifyTokenInputPayload = {
            ...
            access_token: (refreshedToken as { accessToken: string }).accessToken,
            refresh_token: (refreshedToken as { refreshToken: string }).refreshToken,
            expires_in: (refreshedToken as { accessTokenExpires: number }).accessTokenExpires
              ? Math.floor(
                  ((refreshedToken as { accessTokenExpires: number }).accessTokenExpires - Date.now()) / 1000
                )
              : 3600,
