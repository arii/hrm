...
import { SpotifyTokenDeliveryPayloadSchema } from './validation/spotify'

export const createTokenDeliveryHandler = (services: AppServices) => {
  return async (req: Request, res: Response) => {
    try {
      const validationResult = SpotifyTokenDeliveryPayloadSchema.safeParse(
        req.body
      )
      ...
      if (!validationResult.success) {
        logger.warn(
          ...
          'Received a request with an invalid body.'
        )
