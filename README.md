...
That's it. The container will build, install all dependencies (`ppnpm install --frozen-lockfile` and Playwright), and create a `.env.local` file for you. Once the container is ready, you can start the development server:
...
```bash
# Build TypeScript server and Next.js app (optional; ppnpm run start auto-builds if needed)
ppnpm run build
...
`ppnpm run start` now checks for `.env.production` and verifies build artifacts. When `.next/` or
`dist/server.mjs` are missing it runs `ppnpm run build` before launching PM2, so manual builds are
...
```bash
ppnpm run dev                 # Start dev server (Next.js + WebSocket + services)
ppnpm run build               # Build for production
ppnpm run start               # Start production server with PM2
ppnpm run lint                # Run ESLint
ppnpm run lint:fix            # Auto-fix lint issues
ppnpm run format              # Format codebase with Prettier
ppnpm run format:check        # Verify formatting without writing
ppnpm run test:core           # Canonical Playwright suite (chromium baseline screenshots)
ppnpm run test:quick          # Fast smoke run (Playwright, dot reporter)
ppnpm run test:visual:update  # Regenerate baseline screenshots after intentional UI changes
ppnpm run test:visual:headed  # Run Playwright in headed mode for debugging
ppnpm run test:visual:ui      # Launch Playwright interactive UI
ppnpm run test:visual:report  # View the latest Playwright HTML report
ppnpm run test:clean          # Kill stray processes, boot dev server, run baseline tests
ppnpm run test:clean:update   # Clean start + regenerate baseline screenshots
ppnpm run verify:spotify      # Automated Spotify integration health check
ppnpm run kill-all            # Force-stop lingering Node/Chrome processes
ppnpm run mcp:chrome-devtools # Start Chrome DevTools MCP (isolated profile)
```
...
- **Baseline visual regression:** `ppnpm run test:core`
- **Snapshot updates (intentional UI changes):** `ppnpm run test:visual:update`
- **Headed debugging:** `ppnpm run test:visual:headed`
- **Interactive runner:** `ppnpm run test:visual:ui`
- **Fast smoke (under a minute):** `ppnpm run test:quick`
- **Clean environment runs:** `ppnpm run test:clean` / `ppnpm run test:clean:update`
- **Reports:** `ppnpm run test:visual:report`
- **Spotify integration health check:** `ppnpm run verify:spotify`
...
**Best practice:** Run `ppnpm run test:core` before pushing UI changes and regenerate snapshots only after manual review.
...
5. Restart the server and run `ppnpm run verify:spotify` to test the connection.
...
1. **Run the custom server**: Always use `ppnpm run dev` for development to ensure all background services are running.
...
4. **Code Quality**: Run `ppnpm run lint` before committing changes.
5. **Visual Testing**:
   - Run `ppnpm run test:core` before committing UI changes.
   - Use `ppnpm run test:visual:update` only after verifying differences locally.
   - Reach for `ppnpm run test:visual:headed` or `ppnpm run test:visual:ui` when debugging failures.
   - For end-to-end validation, follow with `ppnpm run test:clean` to exercise the dev server startup path.
...
- **Formatting**: This project uses Prettier for code formatting. Please run `ppnpm run format` before submitting a pull request.
- **Linting**: We use ESLint for static analysis. Run `ppnpm run lint` to check for any issues.
...
1. **Spotify token persistence audits** – Exercise `ppnpm run verify:spotify` after redeploys and confirm `logs/spotify_tokens.json` survives restarts.
...
1.  **Build and Start**: Ensure `.env.production` exists, then run `ppnpm run start`. The script verifies build artifacts and kicks off `ppnpm run build` automatically when needed.
...
- **Solution**: Find and kill the process using port 3000, or use `ppnpm run pm2:stop`.

#### Module Resolution Errors

- **Symptom**: `Error [ERR_MODULE_NOT_FOUND]: Cannot find module ...`
- **Solution**: Use `ppnpm run dev` for development, and ensure your `tsconfig.json` is configured for CommonJS modules.
