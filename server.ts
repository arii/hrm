...
import { initializeCoreServices } from './utils/services.js'
...
app
  ...
  .then(async () => {
    ...
    // 2. Initialize Persistent Core Services via the new centralized function
    const { spotifyService, tabataService } = await initializeCoreServices()
    ...
    expressApp.use(async (req: Request, res: Response) => {
      ...
      if (
        ...
        // Asynchronously signal the Spotify service to reload the new token
        // and then force an immediate poll to update the state.
        // No more fragile setTimeout.
        if (spotifyService) {
          await spotifyService.setRefreshToken('signal')
