...
import { getAvailableDevices } from "./services/spotify/spotifyApi.js";
import { UserTokenManager } from "./services/userTokenManager.js";
...
app
  ...
  .then(() => {
    ...
    expressApp.post("/api/spotify/devices", express.json(), async (req: Request, res: Response) => {
      ...
      try {
        const tokenManager = new UserTokenManager(process.env.SPOTIFY_CLIENT_ID || "", process.env.SPOTIFY_CLIENT_SECRET || "");
        const accessToken = await tokenManager.getValidAccessTokenForUser(userId, encryptedRefreshToken);
        if (!accessToken) {
