...
app
  ...
  .then(async () => {
    ...
    // Intercept token delivery POST to update the Spotify service in-memory
    expressApp.post(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          // 3. Respond
          logger.info(
            { subject: tokenPayload.sub ?? tokenPayload.provider },
            'In-memory token-delivery successful'
          )
