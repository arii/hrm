...
const initSocketManager = (
  ...
  const monitor = new ConnectionMonitor(wss)
  monitor.start()

  wss.on('connection', (ws: WebSocket) => {
    const extWs = ws as ExtWebSocket
    extWs.clientId = `user-${Math.random().toString(36).substring(2, 9)}`
    extWs.isAlive = true // Initialize on connect
...
const handleIncomingMessage = (
  ...
  try {
    ...
    switch (message.type) {
      case 'PING': {
        // This application-level PING is maintained for backward compatibility
        // with older clients that may not support the native WebSocket ping/pong
        // frames used by the ConnectionMonitor. It can be considered for
        // deprecation in a future release. No PONG response is sent from here
        // to avoid redundant traffic.
        break
      }
