...
export class SocketManager {
  ...
  private clientSockets = new Map<string, ExtWebSocket>()
  ...
  private handleConnection(ws: ExtWebSocket, req: IncomingMessage) {
    ...
    // Proactively terminate any existing connection for this client ID.
    // This prevents a "last-write-wins" scenario in the clientSockets map
    // and ensures we don't have orphaned, zombie sockets.
    if (this.clientSockets.has(clientId)) {
      const existingSocket = this.clientSockets.get(clientId)
      if (existingSocket && existingSocket !== ws) {
        logger.info(
          { clientId },
          'Terminating previous active connection for this Client ID.'
        )
    ...
    // The 'ws' library uses a ping-pong mechanism for keep-alive.
    // isAlive is set to true on connection and pong events. The server
    // periodically pings clients, and if a client doesn't respond with a
    // pong, isAlive will be false, and the connection is terminated.
    ws.isAlive = true
    ws.on('pong', () => {
      ws.isAlive = true
    })
    ...
    if (existingClient) {
      ...
      this.hrmDataRepository.save(existingClient)
      const oldTimer = this.disconnectionTimers.get(clientId)
      if (oldTimer) {
        clearTimeout(oldTimer)
        this.disconnectionTimers.delete(clientId)
        logger.info(
          { clientId },
          'Reconnection successful, cleared disconnection timer.'
        )
      }
    } else {
  ...
  private handleMessage(ws: ExtWebSocket, message: RawData) {
    try {
  ...
  private terminateAndMigrate(
    ...
    // With the new logic in `handleConnection`, the socket in the map is always
    // the most current one. This function is called when a different client
    // reclaims a deviceId, so we terminate the old client's socket.
    const zombieSocket = this.clientSockets.get(zombieData.clientId)
    if (zombieSocket) {
      logger.info(
        { clientId: zombieData.clientId, deviceId: zombieData.deviceId },
        'Terminating zombie WebSocket connection due to deviceId reclaim.'
      )
      // Use terminate() for immediate, forceful closure.
      zombieSocket.terminate()
    } else {
      logger.warn(
        {
          clientId: zombieData.clientId,
          deviceId: zombieData.deviceId,
          connectedClientsCount: this.clientSockets.size,
        },
        'Could not find the WebSocket to terminate for zombie client.'
      )
    }
