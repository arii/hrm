  ...
  return (
    <Stack spacing={2} sx={{ mt: 2 }}>
      {sessions.map((session) => (
        <Card key={session.id} variant="outlined" sx={{ borderColor: 'rgba(255,255,255,0.1)' }}>
            ...
            >
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  {session.phases.length} Phases • {session.phases.reduce((acc, p) => acc + p.exercises.length, 0)} Exercises
                </Typography>
              </Box>

              <IconButton
                sx={{
                  transform: expandedId === session.id ? 'rotate(180deg)' : 'rotate(0deg)',
                  transition: '0.3s'
                }}
              >
                <ExpandMoreIcon />
