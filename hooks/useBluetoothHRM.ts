...
const useBluetoothHRM = (props: UseBluetoothHRMProps = {}) => {
  ...
  const [poorSignalStrikeCount, setPoorSignalStrikeCount] = useState(0)
  ...
  const updateSignalPeriod = useCallback((newPeriod: number) => {
    ...
    // Track poor signal strikes for warnings
    if (newPeriod > MIN_MISSED_PACKET_THRESHOLD_MS) {
      setPoorSignalStrikeCount((prev) => prev + 1)
    } else {
      setPoorSignalStrikeCount(0)
    }
  }, [])
