...
const useBluetoothHRM = (props: UseBluetoothHRMProps = {}) => {
  ...
  const activeDisconnectListenerRef = useRef<((event: Event) => void) | null>(null)
  ...
  const connectToGattRef = useRef<((device: BluetoothDevice, isReconnect?: boolean) => Promise<boolean>) | null>(null)
  ...
  const stopKeepAlive = useCallback(async () => {
    ...
    if (audioRef.current) {
      try { audioRef.current.pause() } catch (err) { logger.debug({ err }, 'Failed to pause silent audio') }
  ...
  const startKeepAlive = useCallback(async () => {
    ...
    if (!wakeLockRef.current) {
      ...
      } catch (err) { logger.error({ err }, 'Wake lock failed') }
    }
    if (!audioRef.current) {
    ...
    try {
      ...
      if (p instanceof Promise) p.catch(err => logger.debug({ err }, 'Silent audio play failed'))
    } catch (err) { logger.debug({ err }, 'Silent audio failed') }
  }, [])
  ...
  const cleanupGattConnection = useCallback(async () => {
    ...
    if (device) {
      if (activeDisconnectListenerRef.current) {
        device.removeEventListener('gattserverdisconnected', activeDisconnectListenerRef.current)
        activeDisconnectListenerRef.current = null
      }
      if (hrCharacteristicRef.current && hrListenerRef.current) {
        hrCharacteristicRef.current.removeEventListener('characteristicvaluechanged', hrListenerRef.current)
        hrListenerRef.current = null
      }
      if (batteryCharacteristicRef.current && batteryListenerRef.current) {
        batteryCharacteristicRef.current.removeEventListener('characteristicvaluechanged', batteryListenerRef.current)
  ...
  const updateSignalPeriod = useCallback((newPeriod: number) => {
    ...
    if (periodHistory.current.length > ROLLING_AVG_HISTORY_LENGTH) periodHistory.current.shift()
    avgPeriodMs.current = periodHistory.current.reduce((s, v) => s + v, 0) / periodHistory.current.length
  ...
  useEffect(() => {
    ...
    if (status === BluetoothConnectionStatus.CONNECTED) {
      const name = userName || `Bluetooth HRM (${deviceRef.current?.name || 'Unknown'})`
      const age = userAge || 30
      const metadataData: HrmMetadataUpdateData = { maxHr: calculateMaxHr(age), name, age }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (statusRef.current === BluetoothConnectionStatus.CONNECTED && !isDataStale && lastDataTime.current > 0) {
        const threshold = Math.max(avgPeriodMs.current + MISSED_PACKET_THRESHOLD_BUFFER_MS, MIN_MISSED_PACKET_THRESHOLD_MS)
        if (Date.now() - lastDataTime.current > threshold) updateSignalPeriod(Date.now() - lastDataTime.current)
      }
      if (++checkCounter % 2 === 0 && dataLivenessTimeoutMs > 0 && statusRef.current === BluetoothConnectionStatus.CONNECTED && lastDataTime.current > 0) {
  ...
  }, [dataLivenessTimeoutMs, isDataStale, updateSignalPeriod, heartbeatInterval])
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_TESTING === 'true') {
      window.TEST_CONTROLS = { ...window.TEST_CONTROLS, setHrmStatus: setStatus, setCustomHrmStatusMessage: setCustomStatusMessage }
    }
    return () => {
      cleanupGattConnection()
      if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_TESTING === 'true' && window.TEST_CONTROLS) {
  ...
  const handleConnectionError = useCallback((error: unknown) => {
    ...
    if (error instanceof DOMException) {
      if (error.name === 'NotFoundError') msg = BLUETOOTH_MESSAGES.connectionCancelled
      else if (error.name === 'SecurityError') msg = BLUETOOTH_MESSAGES.securityError
      else if (error.name === 'NetworkError') msg = BLUETOOTH_MESSAGES.connectionFailed
      else msg = BLUETOOTH_MESSAGES.bluetoothError(error.name)
    } else if (error instanceof Error) {
      msg = error.message.includes('timeout') ? BLUETOOTH_MESSAGES.connectionTimeout : `Error: ${error.message}`
    }
    setStatus(BluetoothConnectionStatus.ERROR)
  ...
  const reconnect = useCallback((device: BluetoothDevice, reason = 'Connection lost') => {
    if (reconnectAttempts.current >= BLUETOOTH_MAX_RECONNECT_ATTEMPTS) {
      setCustomStatusMessage(BLUETOOTH_MESSAGES.failedToReconnect(BLUETOOTH_MAX_RECONNECT_ATTEMPTS))
    ...
    const isBusy = reason.toLowerCase().includes('busy') || reason.toLowerCase().includes('networkerror')
    setStatus(isBusy ? BluetoothConnectionStatus.CONNECTING : BluetoothConnectionStatus.RECONNECTING)
    setCustomStatusMessage(isBusy ? BLUETOOTH_MESSAGES.deviceBusy(delay, reconnectAttempts.current, BLUETOOTH_MAX_RECONNECT_ATTEMPTS) : BLUETOOTH_MESSAGES.reconnectingAttempt(reason, reconnectAttempts.current, BLUETOOTH_MAX_RECONNECT_ATTEMPTS))

    reconnectTimeoutRef.current = setTimeout(() => {
      if (statusRef.current !== BluetoothConnectionStatus.CONNECTED && !isConnecting.current && !isManualDisconnect.current) {
        connectToGattRef.current?.(device, true).catch(error => {
          logger.warn({ error }, 'Reconnect failed')
          reconnect(device, error instanceof Error ? error.message : String(error))
        })
      }
    }, delay)
  }, [forgetDevice])

  const onDisconnected = useCallback((event: Event | undefined) => {
    const device = (event?.target as BluetoothDevice) ?? deviceRef.current
    if (!device) return
    logger.info({ device: device.name, manual: isManualDisconnect.current, timeout: isTimeoutDisconnect.current }, 'Device disconnected')
    setBatteryLevel(null)
    setStatus(BluetoothConnectionStatus.DISCONNECTED)
  ...
  const connectToGatt = useCallback(async (device: BluetoothDevice, isReconnect = false) => {
    ...
    try {
      ...
      if (previousDevice && previousDevice !== device && activeDisconnectListenerRef.current) {
        previousDevice.removeEventListener('gattserverdisconnected', activeDisconnectListenerRef.current)
      }
      deviceRef.current = device
      if (!isReconnect) {
        setStatus(BluetoothConnectionStatus.CONNECTING)
        setCustomStatusMessage(BLUETOOTH_MESSAGES.connectingToDevice(device.name || ''))
      }

      let server: BluetoothRemoteGATTServer | undefined
      for (let attempt = 1; attempt <= FAST_RECONNECT_MAX_ATTEMPTS; attempt++) {
        try {
          ...
          if ((String(error).includes('busy') || String(error).includes('NetworkError')) && attempt < FAST_RECONNECT_MAX_ATTEMPTS) {
            await new Promise(res => setTimeout(res, FAST_RECONNECT_DELAY_MS))
            continue
          }
          throw error
        }
      ...
      try {
        ...
        const bChar = await bService.getCharacteristic(BATTERY_LEVEL_CHARACTERISTIC_UUID)
        setBatteryLevel((await bChar.readValue()).getUint8(0))
        await bChar.startNotifications()
        const bListener = (e: Event) => setBatteryLevel((e.target as BluetoothRemoteGATTCharacteristic).value!.getUint8(0))
      ...
      const hrListener = (e: Event) => {
        const now = Date.now()
        if (lastDataTime.current > 0) updateSignalPeriod(now - lastDataTime.current)
      ...
      setCustomStatusMessage(BLUETOOTH_MESSAGES.connectedToDevice(device.name || ''))
  ...
  useEffect(() => { connectToGattRef.current = connectToGatt }, [connectToGatt])

  const connectAndStream = useCallback(async (uName?: string, uAge?: number, options: { silent?: boolean } = {}): Promise<boolean> => {
    ...
    try {
      ...
      if (!device && !silent) {
        device = await navigator.bluetooth.requestDevice({ filters: [{ services: [HR_SERVICE_UUID] }], optionalServices: [BATTERY_SERVICE_UUID] })
      }
  ...
  const autoConnect = useCallback(async () => {
    ...
    try {
      ...
      setStatus(BluetoothConnectionStatus.DISCONNECTED)
      setCustomStatusMessage(String(e).includes('No saved device ID') ? null : BLUETOOTH_MESSAGES.autoConnectFailed)
    }
  }, [connectAndStream])
      ...
      if (document.visibilityState === 'visible') {
        if (statusRef.current === BluetoothConnectionStatus.CONNECTED) {
          if (!deviceRef.current?.gatt?.connected) {
            if (deviceRef.current) reconnect(deviceRef.current, 'Background disconnect')
            else autoConnect()
          } else {
            await startKeepAlive()
  ...
  return { connectAndStream, autoConnect, disconnect, forgetDevice, deviceStatus, batteryLevel, isConnected: status === BluetoothConnectionStatus.CONNECTED, isDataStale, isSupported, signalPeriodMs, connectionAttempted }
}
