    ...
    if (lastReadingTime.current) {
      ...
      if (userProfile.age > 0 && userProfile.weight > 0) {
        if (userProfile.gender === 'male') {
          caloriesPerMinute = (MALE_CALS.BASE + (MALE_CALS.HR_COEFF * smoothedHr) + (MALE_CALS.WEIGHT_COEFF * userProfile.weight) + (MALE_CALS.AGE_COEFF * userProfile.age)) / KCAL_PER_KJ;
        } else {
          caloriesPerMinute = (FEMALE_CALS.BASE + (FEMALE_CALS.HR_COEFF * smoothedHr) + (FEMALE_CALS.WEIGHT_COEFF * userProfile.weight) + (FEMALE_CALS.AGE_COEFF * userProfile.age)) / KCAL_PER_KJ;
        }
      }
