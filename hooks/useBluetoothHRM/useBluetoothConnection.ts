// hooks/useBluetoothHRM/useBluetoothConnection.ts
...
export const useBluetoothConnection = (
  onGattServerConnected: (server: BluetoothRemoteGATTServer) => Promise<void>
) => {
  ...
  const connectToGatt = useCallback(
    async (device: BluetoothDevice) => {
      ...
      try {
        ...
        await onGattServerConnected(server!)
    ...
    [onDisconnected, onGattServerConnected]
  )
  ...
  const autoConnect = useCallback(async (): Promise<void> => {
    try {
      ...
      await connectAndStream({ silent: true })
