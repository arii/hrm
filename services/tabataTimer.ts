...
export default class TabataTimer {
  ...
  public getDerivedState(): TimerData {
    ...
    if (mode === 'STOPWATCH')
      return {
        isRunning,
        mode: 'STOPWATCH',
        currentPhase: totalElapsed < 5 ? 'PREPARE' : 'WORK',
        timeRemaining: 0,
    ...
    if (totalElapsed < 5)
      return {
        ...
        timeRemaining: Math.ceil(5 - totalElapsed),
        timeElapsed: Math.floor(totalElapsed),
        cycle: 1,
        totalCycles: config.totalCycles,
        soundEventId: 0,
    ...
    if (workoutTime >= cycleDur * config.totalCycles)
      return {
        isRunning: false,
        mode: 'TABATA',
    ...
    return {
      ...
      timeRemaining: Math.ceil(
        isWork ? config.workDuration - cycleTime : cycleDur - cycleTime
      ),
      timeElapsed: Math.floor(totalElapsed),
      cycle: cycleIndex + 1,
      totalCycles: config.totalCycles,
  ...
  public pause() {
    if (!this.persistentState.isRunning || !this.persistentState.startTime)
      return
    this.persistentState.accumulatedElapsed +=
