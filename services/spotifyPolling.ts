...
import { SpotifyData, HrmData, UnifiedStateMessage } from '../types/websocket';
...
class SpotifyPolling {
    ...
    private state: SpotifyData = { trackName: 'Awaiting Login...', artist: '', isPlaying: false };
    ...
    private async refreshAccessToken(initial: boolean = false): Promise<void> {
        if (!this.refreshToken) {
            if (!initial) {
                this.broadcastState({ spotifyData: { trackName: 'Requires Login', artist: 'Please log in via client/control', isPlaying: false } });
            }
            return;
        }
    ...
    private startPolling() {
        ...
        // Poll every 3 seconds for low-latency updates
        this.pollInterval = setInterval(this.getCurrentlyPlaying, 3000); 
        ...
        try {
            ...
            if (response.status === 204) {
                ...
                if (this.lastPlaybackState !== false) {
                    ...
                    this.state = { trackName: 'Nothing is currently playing.', artist: '', isPlaying: false };
            ...
            if (data.item?.id !== this.lastTrackId || data.is_playing !== this.lastPlaybackState) {
    ...
    private async executePlayerCommand(endpoint: string, method: 'POST' | 'PUT') {
        ...
        try {
            ...
            if (response.status === 204) {
                ...
                console.error(`Spotify command failed (${response.status}): ${endpoint}`);
            }
        } catch (error) {
    ...
    // --- HRM Integration ---
    /**
     * Receives HR value from client and broadcasts the updated state.
     */
    public processHeartRate(hrValue: number) {
        // Broadcast the raw HRM data so the dashboard gets the update
        this.broadcastState({ 
            hrmData: { value: hrValue, maxHr: 185 } as HrmData
        });
        
        // Potential future logic: adjust Spotify tempo or volume based on HR zone
    }
}
