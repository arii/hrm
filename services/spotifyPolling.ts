...
export class SpotifyPolling implements SpotifyService {
  ...
  private async executeSpotifyCommand(
    ...
    const { deviceId, volume, playlistUri } = params
    // Note: The Spotify Web API allows player commands (play, pause, next, previous)
    // to be sent without a `device_id` to target the user's currently active device.
    // While the TypeScript SDK's method signatures mark `device_id` as required,
    // passing `undefined` is handled correctly by the underlying `paramsFor` helper,
    // which omits the parameter from the request. This allows us to control the active
    // device and maintain type safety without resorting to `as any`.
    switch (command) {
      case 'PLAY':
