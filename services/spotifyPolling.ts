...
export class SpotifyPolling {
  ...
  private broadcastSpotifyUpdate: (data: SpotifyData) => void
  ...
  private constructor(broadcastSpotifyUpdate: (data: SpotifyData) => void) {
    this.broadcastSpotifyUpdate = broadcastSpotifyUpdate
  ...
  public static async create(
    broadcastSpotifyUpdate: (data: SpotifyData) => void
  ): Promise<SpotifyPolling> {
    const instance = new SpotifyPolling(broadcastSpotifyUpdate)
    ...
    try {
      ...
      if (!playbackState) {
        ...
        if (this.lastPlaybackState !== false) {
          ...
          this.broadcastSpotifyUpdate(this.getState())
        }
        return
      }
