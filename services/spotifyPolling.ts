...
export class SpotifyPolling {
  ...
  constructor(
    ...
    void this.bootstrapFromTokenManager()

    // Start token refresh check loop (Every 55 mins)
    setInterval(() => this.refreshAccessToken(), 1000 * 60 * 55)
  }

  private async bootstrapFromTokenManager() {
    try {
      const accessToken = await this.tokenManager.getValidAccessToken()
      if (accessToken) {
        this.accessToken = accessToken
        this.refreshToken = this.tokenManager.getCurrentRefreshToken()
        debugLog('Loaded existing Spotify token from manager. Starting poller.')
        this.startPolling()
      }
    } catch (error) {
      console.warn('Failed to bootstrap Spotify tokens from manager:', error)
    }
  }
