...
const DATA_PATH = '/usr/src/previous_workouts/exercise_data/workout_history_data_categorized.json';
...
class LegacyAdapter {
  ...
  public async getSessions(): Promise<WorkoutSession[]> {
    ...
    try {
      ...
      Object.values(data.daily_workouts).flat().forEach((entry) => {
      ...
      // Sort: Newest first
      this.cache = sessions.sort((a, b) =>
        new Date(b.startedAt).getTime() - new Date(a.startedAt).getTime()
      );

      console.log(`[LegacyAdapter] Loaded ${this.cache.length} legacy sessions.`);
      return this.cache;
    } catch (error) {
