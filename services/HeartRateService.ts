...
class HeartRateService {
  ...
  endSession(): WorkoutStats | null {
    if (!this.sessionStartTime || !this.userSettings || this.sessionData.length === 0) return null;

    const duration = (Date.now() - this.sessionStartTime) / 1000;
    const averageHr = this.sessionData.reduce((acc, curr) => acc + curr.hr, 0) / this.sessionData.length;

    // A simple calorie burn formula, can be improved
    const caloriesBurned = this.sessionData.reduce((acc, curr) => {
        ...
        // Simplified formula for calories burned per minute
        const caloriesPerMinute = ((-55.0969 + (0.6309 * curr.hr) + (0.1988 * weight) + (0.2017 * age)) / 4.184) / 60;
