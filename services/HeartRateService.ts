...
class HeartRateService {
  ...
  getWorkoutSummary() {
    ...
    const durationInSeconds = ((this.session.endTime || Date.now()) - this.session.startTime) / 1000;
    const averageHr = this.session.hrReadings.reduce((a, b) => a + b, 0) / this.session.hrReadings.length || 0;
    ...
    const caloriesBurned = Math.round((averageHr / 150) * 8 * (durationInSeconds / 60));
