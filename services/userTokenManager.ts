...
export class UserTokenManager {
  ...
  private async refreshTokenForUser(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          `Token refresh failed for user ${userId}: ${response.status} - ${errorBody}`
        )
   ...
   */
  public async getValidAccessTokenForUser(
    userId: string,
    ...
    if (!userPayload && encryptedRefreshToken) {
      ...
      if (refreshToken) {
        console.log(
          `User ${userId} not in memory, attempting token refresh from cookie.`
        )
        const refreshedPayload = await this.refreshTokenForUser(
          userId,
          refreshToken
        )
        userPayload = refreshedPayload ?? undefined
      }
    }
