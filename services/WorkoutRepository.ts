...
const LIVE_DATA_PATH =
  process.env.WORKOUTS_REPO_PATH || path.join(process.cwd(), 'data/workouts')
...
class WorkoutRepository {
  ...
  public async saveSession(session: WorkoutSession): Promise<void> {
    ...
    const datePrefix = new Date(session.startedAt)
      .toISOString()
      .replace(/[:.]/g, '-')
  ...
  public async getAllSessions(): Promise<WorkoutSession[]> {
    ...
    return [...liveSessions, ...legacySessions].sort(
      (a, b) =>
