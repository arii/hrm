...
# Verify LAST_COMMIT exists
if ! git rev-parse --verify ${LAST_COMMIT}^{commit} >/dev/null 2>&1; then
  echo "Error: LAST_COMMIT ${LAST_COMMIT} does not exist in repository"
  exit 1
fi
...
if [ -n "${GITHUB_EVENT_BEFORE}" ] && git rev-parse --verify ${GITHUB_EVENT_BEFORE}^{commit} >/dev/null 2>&1; then
    ...
    if ! PREVIOUS_COMMIT=$(git merge-base ${LAST_COMMIT} ${PULL_REQUEST_BASE_SHA} 2>&1); then
      echo "Warning: Could not determine merge-base, using parent commit"
      PREVIOUS_COMMIT=$(git rev-parse ${LAST_COMMIT}^) || PREVIOUS_COMMIT=${LAST_COMMIT}
    fi
fi
...
if [ "$PREVIOUS_COMMIT" == "$LAST_COMMIT" ]; then
  ...
  if ! CHANGED_FILES=$(git diff --name-only $PREVIOUS_COMMIT $LAST_COMMIT 2>&1); then
    echo "Warning: Could not determine file changes, assuming no changes"
    CHANGED_FILES=""
  fi
fi
...
# Get commit message (safely handle if commit doesn't exist)
if git rev-parse --verify ${LAST_COMMIT}^{commit} >/dev/null 2>&1; then
