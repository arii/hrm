...
1.  **Architecture Decision Record (ADR)**: The `README.md` explains that the server is stateful and cannot be deployed on Vercel, but it doesn't explain _why_ this architectural choice was made. An ADR file (e.g., `docs/adr/001-custom-stateful-server.md`) would provide critical context for future developers.
