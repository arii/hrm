...
The application uses a unified, environment-aware logger located at `lib/logger.ts`. This logger provides a consistent interface for both client and server-side logging, while dynamically selecting the appropriate logging mechanism based on the execution environment.
...
-   **Warning**: Use `logger.warn` to log potential issues that do not necessarily break the application.
    ...
    const checkApiResponse = (response) => {
      if (response.status !== 200) {
        logger.warn('API response was not successful.', { status: response.status })
      }
    }
    ```
...
The `httpLogger` middleware, also exported from `lib/logger.ts`, is used in `server.ts` to automatically log all incoming HTTP requests and their responses.
...
This helps to protect user privacy and prevent security vulnerabilities. For more details, see the redaction configuration in `lib/logger.ts`.
