import type { Meta, StoryObj } from '@storybook/react'
import Dashboard from '@/app/page' // Adjust based on your actual export
import { MockWebSocketProvider } from '../mocks/MockWebSocketProvider'
import { ServerMessage } from '@/types/websocket'

const RACE_SCENARIO: ServerMessage[] = [
  {
    type: 'HRM_UPDATE',
    payload: [
      { clientId: '1', value: 120, maxHr: 190, name: 'Runner A', calories: 10, timestamp: Date.now() },
    ],
  },
  {
    type: 'HRM_UPDATE',
    payload: [
      { clientId: '1', value: 145, maxHr: 190, name: 'Runner A', calories: 20, timestamp: Date.now() },
    ],
  },
  {
    type: 'HRM_UPDATE',
    payload: [
      { clientId: '1', value: 165, maxHr: 190, name: 'Runner A', calories: 30, timestamp: Date.now() },
    ],
  },
]

const meta = {
  title: 'Pages/Dashboard',
  component: Dashboard,
} satisfies Meta<typeof Dashboard>

export default meta
type Story = StoryObj<typeof meta>

export const LiveSimulation: Story = {
  render: () => (
    <MockWebSocketProvider scenario={RACE_SCENARIO} interval={800}>
      <Dashboard />
    </MockWebSocketProvider>
  ),
}
