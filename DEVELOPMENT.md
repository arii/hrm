...
This project leverages AI, specifically Google's Gemini models, to assist with and automate various development tasks.
...
To improve performance and reduce costs for tasks involving large amounts of context (like code reviews), the Gemini client supports context caching. This feature uploads the context files to a temporary cache on Google's servers and reuses it for subsequent API calls within a short timeframe.
...
- **`GEMINI_CACHE_FILE_THRESHOLD`**: In the `gemini-coder.yml` workflow, this environment variable controls the minimum number of files required to trigger context caching. The default is `5`.
