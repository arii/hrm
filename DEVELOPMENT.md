...
When creating new internal modules or utilities (e.g., a new file in `lib/` or a new subdirectory in `utils/` like `utils/core`) that **only** use built-in Node.js APIs or dependencies already listed in `package.json`, changes to `package.json` or `pnpm-lock.yaml` are **not** required. This includes the `utils/core` directory, which is the designated location for shared, application-wide utility functions.
