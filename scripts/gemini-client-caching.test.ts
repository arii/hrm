...
import { main } from './gemini-client'; // Adjust this import to your actual file structure
import path from 'path';

// Mock the server-side SDKs
jest.mock('@google/generative-ai/server', () => ({
  GoogleAIFileManager: jest.fn().mockImplementation(() => ({
    uploadFile: jest.fn((filePath) => Promise.resolve({
      file: {
...
describe('Gemini Client Caching Logic', () => {
  ...
  it('should use the standard text-based flow when --use-cache is not provided', async () => {
    ...
    process.argv = ['node', 'gemini-client.ts', '--task', 'test task', '--context', 'file1.txt'];
  ...
  it('should attempt to create and use a cache when --use-cache is provided', async () => {
    ...
    process.argv = ['node', 'gemini-client.ts', '--task', 'test task', '--context', 'file1.txt,file2.txt', '--use-cache'];
    ...
    const fileManagerInstance = (GoogleAIFileManager as jest.Mock).mock.instances[0];
    expect(fileManagerInstance.uploadFile).toHaveBeenCalledTimes(2);
    expect(fileManagerInstance.uploadFile).toHaveBeenCalledWith(path.resolve(process.cwd(), 'file1.txt'), expect.any(Object));
    expect(fileManagerInstance.uploadFile).toHaveBeenCalledWith(path.resolve(process.cwd(), 'file2.txt'), expect.any(Object));

    // Verify that the cache manager was called to create a cache
    const cacheManagerInstance = (GoogleAICacheManager as jest.Mock).mock.instances[0];
  ...
  it('should use the configurable TTL from environment variables if set', async () => {
    ...
    process.argv = ['node', 'gemini-client.ts', '--task', 'test task', '--context', 'file1.txt', '--use-cache'];
    jest.spyOn(require('fs/promises'), 'readFile').mockResolvedValue('file content');

    await main();

    const cacheManagerInstance = (GoogleAICacheManager as jest.Mock).mock.instances[0];
    expect(cacheManagerInstance.create).toHaveBeenCalledWith(expect.objectContaining({
      ttlSeconds: 7200,
    }));
  });
  ...
  it('should fall back to standard text context if cache creation fails', async () => {
    ...
    const cacheManagerInstance = (GoogleAICacheManager as jest.Mock).mock.instances[0] || new (GoogleAICacheManager as any)();
    (cacheManagerInstance.create as jest.Mock).mockRejectedValue(new Error('Cache creation failed'));
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to initialize cache. Falling back to standard text context.'));

    // Verify that the standard model was called (and not the cached one)
    const genAIInstance = (require('@google/generative-ai').GoogleGenerativeAI as jest.Mock).mock.instances[0];
    expect(genAIInstance.getGenerativeModel).toHaveBeenCalled();
    expect(genAIInstance.getGenerativeModelFromCachedContent).not.toHaveBeenCalled();
