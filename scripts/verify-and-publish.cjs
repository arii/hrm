...
function publishStatus(sha, context, state, description) {
  ...
  if (!token) {
    console.warn(
      `⚠️  Skipping GitHub publish for ${context} (No GITHUB_TOKEN found)`
    )
...
async function main() {
  ...
  try {
    ...
    console.error(
      '❌ Critical: Failed to parse test report JSON:',
      error.message
    )
    process.exit(1)
  }
  ...
  if (report.suites)
    report.suites.forEach((s) =>
      processSuite(s, path.basename(s.title, '.spec.ts'))
    )
  ...
  promises.push(
    publishStatus(
      ...
      globalSuccess ? 'Verified Safe' : 'Tests Failed'
    )
  )
