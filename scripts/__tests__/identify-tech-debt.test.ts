...
describe('identify-tech-debt', () => {
    ...
    const mockGenerateContent = geminiClient.generateContentWithFallback as jest.Mock;
    ...
    beforeEach(() => {
      ...
      process.argv = ['node', 'identify-tech-debt.ts', '--diff-file', 'diff.txt', '--output', 'output.json'];
    });
    ...
    describe('isTechDebtResponse', () => {
        it('should return true for valid data', () => {
          const data = {
            issues: [
              { title: 't', description: 'd', fingerprint: 'f' },
            ],
          };
    ...
    it('should successfully process a valid response from the AI', async () => {
        const validResponse = {
            issues: [{ title: 'Test Issue', description: 'A test issue', fingerprint: 'test-fingerprint' }],
          };
        ...
        expect(mockWriteFile).toHaveBeenCalledWith(expect.any(String), JSON.stringify(validResponse, null, 2));
      });
      ...
      it('should handle a malformed JSON response from the AI', async () => {
        ...
        expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('Failed to parse valid technical debt JSON'));
        expect(mockWriteFile).toHaveBeenCalledWith(expect.any(String), JSON.stringify({ issues: [] }, null, 2));
      ...
      it('should exit if GEMINI_API_KEY is not set', async () => {
        ...
        expect(mockConsoleError).toHaveBeenCalledWith('Error: GEMINI_API_KEY environment variable is not set.');
        expect(mockExit).toHaveBeenCalledWith(1);
      });
