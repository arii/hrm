...
if git diff --quiet "$MERGE_BASE" HEAD -- .; then
  ...
  if [ -n "$GITHUB_REPOSITORY" ]; then
    ...
    # Fallback if variable missing, assuming current repo context for gh
    echo "::warning::GITHUB_REPOSITORY not set. Using implicit repo context."
    # gh api "pulls/$PR_NUMBER/commits" might not work without repo context if not in a git repo.
    # But usually this runs in a git repo.
    # However, 'gh api' endpoint for pulls is /repos/{owner}/{repo}/pulls...
    # Without GITHUB_REPOSITORY, we might be in trouble if gh can't infer it.
    # Let's assume gh can infer from git remote if available, or fail.
    # But to be safe, we really want GITHUB_REPOSITORY.
