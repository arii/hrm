...
describe('WebSocket Full Integration Test', () => {
  ...
  beforeAll((done) => {
    ...
    serverProcess.stderr?.on('data', (data: Buffer) => console.error(`[Server ERR]: ${data.toString().trim()}`));
        ...
        if (res.statusCode === 200) {
          ...
          // It can be unhealthy if Spotify isn't configured, but we check for 503 as a valid "running" state.
           if (res.statusCode === 503) {
            console.log('Server is running but unhealthy (as expected without Spotify).');
            clearInterval(interval);
            clearTimeout(timeout);
            done();
           }
    ...
    const timeout = setTimeout(() => {
        clearInterval(interval);
        done(new Error(`Server failed to start or respond to health check in 50 seconds.`));
    }, 50000);
  });
  ...
  it('should handle a full user workflow: connect, send HR, start timer, receive updates, stop timer', (done) => {
    ...
    const runWorkflow = async () => {
      ...
      // 2. Send HR data
      const hrmInput: HrmInputMessage = { type: 'HRM_INPUT', data: { value: 135, name: 'Workflow Test' }};
      ...
      // 3. Start the timer
      const startCommand: TimerCommandMessage = { type: 'TIMER_COMMAND', command: 'START' };
      ...
      // 4. Stop the timer
      const stopCommand: TimerCommandMessage = { type: 'TIMER_COMMAND', command: 'STOP' };
