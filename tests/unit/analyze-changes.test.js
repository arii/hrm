...
const scriptPath = path.resolve(__dirname, '../../.github/scripts/analyze-changes.sh');
...
describe('analyze-changes.sh', () => {
  ...
  it('should use GITHUB_EVENT_BEFORE for PREVIOUS_COMMIT on a normal push', async () => {
    ...
    const previousCommitLine = envFileContent.split('\n').find(line => line.startsWith('PREVIOUS_COMMIT='));
    const previousCommit = previousCommitLine ? previousCommitLine.split('=')[1] : '';
  ...
  it('should use the merge-base for PREVIOUS_COMMIT on a rebased push', async () => {
    ...
    const mergeBase = await execAsync(`git merge-base ${headSha.stdout.trim()} ${baseSha.stdout.trim()}`);
