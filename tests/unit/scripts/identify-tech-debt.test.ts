...
import { isTechDebtResponse, main } from '../../../scripts/identify-tech-debt'
  ...
  describe('main', () => {
    ...
    it('should exit if GEMINI_API_KEY is not set', async () => {
      ...
      expect(mockStderr).toHaveBeenCalledWith(
        'An unexpected error occurred during the analysis process:',
        expect.any(Error)
      )
      expect(mockExit).toHaveBeenCalledWith(1)
    })
    ...
    it('should process files and write aggregated issues', async () => {
      ...
      const mockApiResponse1 = {
        issues: [
          { title: 'Issue 1', description: 'Desc 1', fingerprint: 'fp1' },
        ],
      }
      const mockApiResponse2 = {
        issues: [
          { title: 'Issue 2', description: 'Desc 2', fingerprint: 'fp2' },
        ],
      }
      ...
      ;(geminiClient.JsonProcessor as jest.Mock).mockReturnValue(
        mockJsonProcessor
      )
      ...
      expect(writeFile).toHaveBeenCalledWith(
        expect.any(String),
        JSON.stringify(
          { issues: [...mockApiResponse1.issues, ...mockApiResponse2.issues] },
          null,
          2
        )
      )
      expect(mockStdout).toHaveBeenCalledWith(
        expect.stringContaining('Technical debt analysis complete.')
      )
    })
    ...
    it('should handle file read errors gracefully', async () => {
      ...
      const mockJsonProcessor = {
        process: jest
          .fn()
          .mockReturnValue({ success: true, data: mockApiResponse }),
      }
      ;(geminiClient.JsonProcessor as jest.Mock).mockReturnValue(
        mockJsonProcessor
      )
      ;(
        geminiClient.generateContentWithFallback as jest.Mock
      ).mockResolvedValue(JSON.stringify(mockApiResponse))

      await main()

      expect(mockStderr).toHaveBeenCalledWith(
        'Error processing file bad.ts:',
        expect.any(Error)
      )
      expect(writeFile).toHaveBeenCalledWith(
        expect.any(String),
    ...
    it('should handle invalid JSON from API gracefully', async () => {
      ...
      const mockJsonProcessor = {
        process: jest
          .fn()
          .mockReturnValue({ success: false, data: { error: 'bad json' } }),
      }
      ;(geminiClient.JsonProcessor as jest.Mock).mockReturnValue(
        mockJsonProcessor
      )
      ;(
        geminiClient.generateContentWithFallback as jest.Mock
      ).mockResolvedValue('{ not_json: "true" }')

      const mockWarn = jest.spyOn(console, 'warn').mockImplementation(() => {})
      await main()

      expect(mockWarn).toHaveBeenCalledWith(
        'Warning: Failed to parse valid technical debt JSON for file1.ts.'
      )
      expect(writeFile).toHaveBeenCalledWith(
        expect.any(String),
