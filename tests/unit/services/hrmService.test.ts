...
describe('HrmService', () => {
  ...
  describe('initializeClient', () => {
    it('should initialize a new client if one does not exist', () => {
      ...
      expect(hrmDataRepository.save).toHaveBeenCalledWith({
        clientId: 'client1', value: 0, maxHr: 185, age: 30, calories: 0,
      });
      expect(logger.info).toHaveBeenCalledWith({ clientId: 'client1' }, 'Initialized new HRM client session.');
    });

    it('should not initialize a new client if one already exists', () => {
      const existingClient: HrmStreamData = {
        clientId: 'client1', value: 120, maxHr: 185, age: 30, calories: 100,
      };
      ...
      expect(logger.info).toHaveBeenCalledWith({ clientId: 'client1' }, 'HRM client reconnected with existing session.');
    });
  });
  ...
  describe('updateMetadata', () => {
    it('should update the metadata for an existing client', () => {
      const existingData: HrmStreamData = {
        clientId: 'client1', value: 120, maxHr: 185, age: 30, calories: 100, name: 'Old Name',
      };
      hrmDataRepository.save(existingData);
      hrmService.updateMetadata('client1', { name: 'New Name', age: 35 });
      expect(hrmDataRepository.save).toHaveBeenCalledWith({
        ...existingData, name: 'New Name', age: 35,
      });
    });

    it('should not overwrite a real name with a default name', () => {
      const existingData: HrmStreamData = {
        clientId: 'client1', value: 120, maxHr: 185, age: 30, calories: 100, name: 'Real Name',
      };
      hrmDataRepository.save(existingData);
      (hrmDataRepository.save as jest.Mock).mockClear();
      hrmService.updateMetadata('client1', { name: 'user-123' });
      expect(hrmDataRepository.save).toHaveBeenCalledWith(expect.objectContaining({ name: 'Real Name' }));
    });
  });
  ...
  describe('processHrmInput', () => {
    ...
    it('should reject client-calculated calories if there is a large discrepancy', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(expect.any(Object), 'Large calorie discrepancy detected. Rejecting client update.');
      const savedData = (hrmDataRepository.save as jest.Mock).mock.calls[1][0];
  ...
  describe('cleanupClient', () => {
    it('should cleanup the client data', () => {
      hrmDataRepository.save({ clientId: 'client1', value: 1, age: 1, calories: 1, maxHr: 1 });
