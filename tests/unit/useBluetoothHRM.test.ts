...
describe('useBluetoothHRM', () => {
  ...
  /** Helper to simulate a heart rate measurement event from the device */
  const simulateHeartRateEvent = (value: number) => {
    const event = {
      target: {
        value: {
          // Typically, the first byte is flags, the second is the HR value
          getUint8: (index: number) => (index === 1 ? value : 0),
        },
      },
    }
    // Find the 'characteristicvaluechanged' event listener and invoke it
