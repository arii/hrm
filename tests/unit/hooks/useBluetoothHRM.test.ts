...
describe('useBluetoothHRM', () => {
  ...
  beforeEach(() => {
    ...
    mockUseBluetoothConnection = jest
      .spyOn(useBluetoothConnection, 'useBluetoothConnection')
      .mockReturnValue({
    ...
    mockUseGattSubscription = jest
      .spyOn(useGattSubscription, 'useGattSubscription')
      .mockReturnValue({
        value: null,
        error: null,
      })

    mockUseDataLiveness = jest
      .spyOn(useDataLiveness, 'useDataLiveness')
      .mockReturnValue({
        isDataStale: false,
      })

    mockUseSignalQuality = jest
      .spyOn(useSignalQuality, 'useSignalQuality')
      .mockReturnValue({
        signalPeriodMs: 0,
        resetSignalQuality: jest.fn(),
      })

    mockUseWebSocket = jest
      .spyOn(WebSocketContext, 'useWebSocket')
      .mockReturnValue({
        sendData: jest.fn(),
  ...
  it('should parse and forward heart rate data on new GATT value', () => {
    ...
    ;(useGattSubscription.useGattSubscription as jest.Mock).mockImplementation(
      (props) => {
        if (props.serviceUuid === 'heart_rate') {
          onValueChangeCallback = props.onValueChange as (
            value: DataView
          ) => void
        }
  ...
  it('should send metadata when connection is established', () => {
    ...
    const { rerender } = renderHook(
      ({ isConnected }) => {
        ;(
          useBluetoothConnection.useBluetoothConnection as jest.Mock
        ).mockReturnValue({
          ...mockUseBluetoothConnection.mock.results[0].value,
          isConnected,
