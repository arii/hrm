...
test.describe('Multi-User Dashboard Visuals', () => {
  test('Dashboard with 4 mock users streaming HR data', async ({
    page,
    context,
  }) => {
    ...
    await expect(mockPage.locator('text=Server Status: Connected')).toBeVisible(
      { timeout: 10000 }
    )
    ...
    await expect(page.locator('[data-testid^="hr-tile-"]')).toHaveCount(1, {
      timeout: 15000,
    })
