 ...
 */
async function verifyAuthConfiguration(page: Page): Promise<{
 ...
 */
async function verifyDebugEndpoints(page: Page): Promise<{
  pingOk: boolean
  sessionOk: boolean
}> {
  const baseUrl = getBaseURL()
 ...
 * @param page - The Playwright Page object
 */
async function verifyNoStateCookieError(page: Page): Promise<void> {
 ...
 */
async function verifySpotifyTokenStatus(page: Page): Promise<{
  status: string
