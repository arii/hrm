...
test.describe('Spotify OAuth Integration (Local)', () => {
  ...
  test.beforeAll(async () => {
    if (!CHROME_PROFILE) {
      console.warn('⚠️  No CHROME_PROFILE_PATH set. Test will run with a fresh profile (login may be required).');
    } else if (!fs.existsSync(CHROME_PROFILE)) {
      console.warn(`⚠️  Profile path not found: ${CHROME_PROFILE}. Test will run with fresh profile.`);
    }
  });
  ...
  test('verify authenticated session and token exchange', async () => {
    ...
    const userDataDir = CHROME_PROFILE || path.join(os.tmpdir(), 'playwright-temp-profile');
    ...
    if (EXPECTED_USER) {
      ...
      const debugAuthResponse = await page.request.get(`${BASE_URL}/api/debug/auth-check`);
