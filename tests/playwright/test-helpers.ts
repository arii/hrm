...
export const replaceIframeWithStableWorkout = async (page: Page) => {
  try {
    // Wait for the iframe to be present, but with a short timeout.
    // This allows us to proceed even if the iframe is blocked by CSP.
    await page.waitForSelector('iframe', { state: 'attached', timeout: 3000 });

    // After the iframe is available, replace its content.
    await page.evaluate(() => {
    ...
    console.warn(
      'Could not find or modify iframe, possibly due to CSP. Continuing test with placeholder content.'
    );
    // If the iframe is blocked or fails to load, the skeleton loader will be shown.
    // The test can still proceed, and the visual snapshot will capture the skeleton state.
  }
};
