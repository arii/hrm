...
test.describe('Bluetooth HRM Connection', () => {
  ...
  test('successfully connects to a new device via requestDevice', async ({ connectPage }) => {
    ...
    await connectPage.evaluate(() => {
      // @ts-ignore
      navigator.bluetooth._simulateHeartRate(78)
    })
  ...
  test('robustness: handles stale connection by forgetting and prompting', async ({ connectPage }) => {
    ...
    await connectPage.context().addCookies([{ name: 'hrm_device_id', value: 'stale-device-id', domain: 'localhost', path: '/' }])
    await connectPage.evaluate(() => {
      // @ts-ignore
      const badDevice = new window.MockBluetoothDevice('stale-device-id', 'Stale HRM')
      // @ts-ignore
    ...
    await connectPage.evaluate(() => {
      // @ts-ignore
      navigator.bluetooth._simulateHeartRate(88)
    })
  ...
  test('Reset Server button triggers device forget', async ({ connectPage }) => {
    ...
    await connectPage.evaluate(() => {
        // @ts-ignore
        navigator.bluetooth._simulateHeartRate(75)
    })
