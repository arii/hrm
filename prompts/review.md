...
```json
{
  ...
  "artifacts": [
    {
      "title": "Brief, actionable title for an issue",
      "description": "Detailed markdown description of the technical debt, finding, or suggestion. This will be used to create a GitHub issue.",
      "category": "technical-debt" | "refactor-needed" | "security" | "performance" | "documentation"
    }
  ]
}
```
...
- If you identify technical debt, a potential refactoring, or a non-blocking issue that should be tracked, create an artifact for it.
- Do not create artifacts for minor nits or stylistic issues that can be fixed directly in the PR.
- The `title` should be a concise summary of the issue.
- The `description` should provide enough context for another developer to understand and address the issue.
